...
func TestDailyRunUsesUKTime(t *testing.T) {
	utcSevenAMWinter := time.Date(2026, time.January, 15, 7, 0, 0, 0, time.UTC)
	if isDailyRunDue(utcSevenAMWinter.In(londonLocation)) {
		t.Fatal("07:00 UK in winter should not be due")
	}

	utcSevenAM := time.Date(2026, time.July, 15, 7, 0, 0, 0, time.UTC)
	if !isDailyRunDue(utcSevenAM.In(londonLocation)) {
		t.Fatal("07:00 UTC should be 08:00 UK in summer and due")
	}

	utcEightAMSummer := time.Date(2026, time.July, 15, 8, 0, 0, 0, time.UTC)
	if !isDailyRunDue(utcEightAMSummer.In(londonLocation)) {
		t.Fatal("08:00 UTC should be 09:00 UK in summer and should catch up")
	}
}
